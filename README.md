...
- **内容筛选**：图文/收藏、视频/小视频、转发、纯文字、专栏、直播/卡片
- **导出格式**：HTML / JSON / CSV 可勾选
- **媒体打包**：图片、表情、视频/音频可选（视频为 DASH 分离流）
...
B站视频为 DASH 分离流（`_video.m4s` + `_audio.m4s`）。安装 ffmpeg 后（Windows：`winget install Gyan.FFmpeg`），解压 ZIP 双击「合并视频.bat」即可合并为 `<BV号>.mp4`。
